...
elsewhere), C# in `repro/csdiv/Program.cs` (console app referencing
`telnet_cs/telnet_cs.csproj`, `MemStream` implements the real
...
(plus the earlier `repro/py_d4_raisevssilent.py`), C# in
`repro/cs_wire.log` (labeled `read`/`readS` runs of
`repro/csdiv`, rebuilt against current sources) and the named xUnit
...
- Proof: `repro/py_d1_usdisable.py` →
  `repro/py_d1_usdisable.log`: a client sending `WILL SGA` then
  `WONT SGA` gets `-> True` on each line with
  `writes= ['fffb03', 'fffc03']` (both bytes go out immediately).
  `repro/csdiv` → `repro/cs_negqueue.log`:
...
- Code: withdrawing our own request (`OfferDisable`,
  `telnet_cs/Protocol/NegotiationState.cs:322-327`) calls
  `InitiateDisableLocked` with `gateOnOutstanding: true`, so the
  `WantYes` branch (`:449-479`, documented `:441-448`) sets
  `queued[option] = true` and returns null. Telling the peer to stop
  (`RequestDisable`, `:253-260`) passes `gateOnOutstanding: false`, so a
...
- Proof: `repro/py_d2_charsetsim.py` →
  `repro/py_d2_charsetsim.log`: with `pending_option[SB+CHARSET]`
  ...
  Cs `repro/cs_charsetsim.log` (`charsetSim`: `IsServerRole=true`,
  `CharsetRequestPending=true`, same bytes): `writes=FFFA2A03FFF0`
  (REJECTED). A client in the same situation answers ACCEPTED like
  telnetlib3 (`repro/cs_clientsim.log`, `clientSim` mode:
  `writes=FFFA2A025554462D38FFF0`).
- Code: `telnet_cs/IO/ByteStreamHandler.cs:2341-2348`
...
- Code: `telnet_cs/IO/ByteStreamHandler.cs:130-218` (`InSynchDiscard`,
  ...
  `telnet_cs/Client/Client.Connect.cs:27-30` ("no opt-out").
...
- Proof: `repro/py_d6_repeatrefuse.log` (same run prints
  `TelnetClient.__init__` defaults): `cols: int = 80, rows: int = 25`.
  `repro/cs_nawsdefault.log`: `effective0x0=0x0`,
  `frame=FFFA1F00000000FFF0` (0,0 sent as-is).
- Code: `telnet_cs/Protocol/NawsProtocol.cs:18-21` (`GetEffectiveSize`
  clamps like telnetlib3's `max(min(65535,v),0)` but a 0 dimension is
  sent as-is, never mapped to a console probe); `Client.Connect.cs:285`
  `_terminalTypeDefault = "unknown"` and `:300`
  `_terminalSpeedDefault = "38400,38400"` already match telnetlib3's
  (`term='unknown'`, `tspeed=(38400,38400)` per
  `repro/py_d5_defaults.log`); `TelnetClientOptions.cs:78-84`
...
- Proof: `repro/py_d8_tspeed.py` →
  `repro/py_d8_tspeed.log`: with send policy `(rx, tx) = (111, 222)`,
  ...
  (`repro/cs_tspeed.log`, `tspeed` mode) — but Cs documents that
...
- Code: telnetlib3 `_handle_sb_tspeed` (`stream_writer.py:2479-2515`):
  ...
  `"<tx>,<rx>"` (`TerminalSpeedProtocol.cs:4-6`; `Validate` names
  `parts[0]` tx and `parts[1]` rx at `:34-35`; SEND-reply log at
  `ByteStreamHandler.cs:1870-1878`; `ClientTerminalSpeed` doc at
  `ServerSession.Collectors.cs:136-140`), matching RFC 1079 §4
...
- Proof: `repro/py_d6_qbit.py` → `repro/py_d6_qbit.log`:
...
- Code: `telnet_cs/Protocol/NegotiationState.cs:360-363` (`WantNo` +
  queued swallows a positive into `Yes` silently), `:367-370`
  (`WantYes` + queued answers the positive with the refuse verb),
  `:364-366` (`WantNo` settles to `No` silently), `:430-432`
...
- Proof: `repro/py_d7_environ.py` →
  `repro/py_d7_environ.log`: `VAR K VALUE a ESC VALUE b` decodes
...
- Code: `telnet_cs/Protocol/EnvironmentProtocol.cs:323-340` (escape
  `VAR`/`VALUE`/`ESC`/`USERVAR`), `:244-273` (unescape; trailing `ESC`
  contributes no byte at `:252`; only unescaped bytes delimit).
...
- Proof: `repro/py_d8_malformed.py` →
  `repro/py_d8_malformed.log`: `LINEMODE` unknown sub, empty
  ...
  `NotImplementedError`. Earlier `repro/py_d4_raisevssilent.log`:
  ...
  one of these with `data=` and `writes=` empty (`repro/cs_wire.log`
  ...
  no `IS`, no `WONT`).
- Code: `telnet_cs/IO/ByteStreamHandler.cs:2303-2315`
  (`ReplyLineflowAsync`: shape and agreement gates), `:2635-2673`
  (`ReplyLinemodeAsync` dispatch at `:2635-2651`, `MODE` shape and
  agreement gates in `ReplyModeAsync`), `:2422-2468`
  (`ReplyCharsetAnswerAsync`: ACCEPTED/REJECTED, then table verbs
  logged and ignored at `:2454-2468`, never answered), `:1889-1898`
  (`STATUS SEND` without agreement ignored). The one deliberate
  exception is the SLC triplet shape (`ByteStreamHandler.cs:2784`
...
- Code: `telnet_cs/IO/ByteStreamHandler.cs:3002-3018` (acts on
  `WILL`/`WONT TM` only with an outstanding `DO` — gate at `:3002` —
...
- Proof: `repro/py_d10_echo.py` →
  `repro/py_d10_echo.log`: server-role inbound `WILL ECHO`
  ...
  is silent (`repro/cs_wire.log` run `d10-willecho-server`), and
...
- Code: `telnet_cs/IO/ByteStreamHandler.cs:3045-3049` (server-role
  `WILL ECHO` swallowed), `:3342-3360` (`AgreeEcho`: WILL answered
  unless we already echo; DO answered from `AllowRemoteEcho` unless the
  peer already echoes; `OfferEcho` defaults on in
  `TelnetServerOptions.cs:29`, wired at `ServerSession.cs:453`).
...
- Proof: `repro/py_d11_logout.py` →
  `repro/py_d11_logout.log`: server-role inbound `WILL LOGOUT`
  fires the callback (`[b'\xfb']`) and sends nothing. C# sends `DONT`
  (`repro/cs_wire.log` run `d11-willlogout-server`:
...
- Code: `telnet_cs/IO/ByteStreamHandler.cs:3021-3043` (client-role
  `WILL`/`DO` handling, server-role `DO` close); a server-role `WILL`
  falls through to the generic table (`:3051-3059`), which refuses an
  unwanted offer, and the refusal goes out at `:3095-3101` (`DONT`).
...
- Proof: `repro/py_d12_mccp.py` →
  `repro/py_d12_mccp.log`: `IAC SB MCCP2 IAC SE` with no
  agreement activates immediately (`activated= True`,
  `callback= [True]`, `writes= []`). C# stays silent and plaintext
  (`repro/cs_wire.log` run `d12-mccp2`): pinned by
...
- Code: `telnet_cs/IO/ByteStreamHandler.cs:2235-2239`
   (`MccpStartAllowed`: opted in, not over TLS, and agreed by either
  side — shared by the empty and padding-carrying forms), `:1805-1836`
  (direction: only a client inflates MCCP2, only a server MCCP3; wrong
  direction never arms), `:1088-1106` with deferred flush at
  `:1263-1279` (a corrupt stream is shut down with `WONT`/`DONT` so the
...
- Proof: `repro/py_d13_naws.py` →
  `repro/py_d13_naws.log`: the strict 4-byte shape is accepted;
  the 5-byte verb-first shape raises `struct.error` out of `feed_byte`.
  C# accepts both and stays silent on the wire (`repro/cs_wire.log`
  ...
  (strict 4-byte shape).
- Code: `telnet_cs/Server/ServerSession.Collectors.cs:1425-1448`
...
- Proof: `repro/py_d14_tspeed.py` →
  `repro/py_d14_tspeed.log`: the `signed-plus` case (`+9600,9600`)
...
- Code: `telnet_cs/Protocol/TerminalSpeedProtocol.cs:30-42`
  ...
  send compliance, values otherwise preserved) with `NormalizeRate` at `:62-79`. telnetlib3
...
- Proof: `repro/py_d15_sndloc.py` →
  `repro/py_d15_sndloc.log`: `handle_send_sndloc()` returns `''`.
...
- Code: `telnet_cs/IO/ByteStreamHandler.cs:2494-2499`
  (`SendLocationPayloadAsync`: `ThrowIfNullOrWhiteSpace`), receive at
  `:2285-2291` plus `ServerSession.Collectors.cs:1725-1732`. telnetlib3
...
- Proof: `repro/py_d16_msdp.py` →
  `repro/py_d16_msdp.log`: `{'K': ('a', 'b')}` encodes to
...
- Code: `telnet_cs/Protocol/MudProtocol.cs:242-256` (non-enumerable
...
- Proof: `repro/py_d17_aardwolf.py` →
  `repro/py_d17_aardwolf.log`: `[0x41]` decodes to
...
- Code: `telnet_cs/Protocol/MudProtocol.cs:575-590` (length switch;
...
- Proof: `repro/py_d18_syncterm.py` →
  `repro/py_d18_syncterm.log`: `\x1b[0;10000 D` detects as
...
- Code: `telnet_cs/Protocol/SyncTermFont.cs:121-137` (accumulate,
  reject above 9999) with `DetectEncoding` at `:68-101` continuing the
...
- Proof: `repro/py_d19_zmp.py` →
  `repro/py_d19_zmp.log` (real `TelnetClient.on_zmp`, writer
...
- Code: `telnet_cs/IO/ByteStreamHandler.cs:2118-2176` (approved is
...
- Proof: `repro/py_d20_ttype.py` →
  `repro/py_d20_ttype.log` (real `TelnetServer.on_ttype`,
...
- Code: `telnet_cs/Server/ServerSession.Collectors.cs` (empty answers
  end the turn without advancing at `:1552-1558`; the returned list
  ends at the first repeat — first-entry loop or previous-entry repeat
  (terminator excluded) at `:1600-1612` and `:609-618` — while a
  third-slot `MTTS` vector stops the cycle but is kept in the chain;
  past-`TerminalTypeLoopMax` (`:328`) the over-cap answer is recorded
  once into the overflow slot and the cycle stops at `:1576-1588`).
...
- Proof: `repro/py_d21_charset.py` →
  `repro/py_d21_charset.log` (real client selection policy
  ...
  **empty** body (`writes= ['fffa2a02fff0']` — unresolvable alias falls
  ...
  (`repro/cs_wire.log` run `d21-request-usascii`:
  ...
  fallthrough).
- Code: `telnet_cs/Protocol/CharsetProtocol.cs:48-71` (`BuildRequest`
  separator fallback), `:96-123` (`ParseRequest` space re-split),
  `:266-328` (alias superset incl. `USASCII` and `cpNNNN`);
  `telnet_cs/IO/ByteStreamHandler.cs:2357-2366` (selector, null →
  `REJECTED`), `:2424-2437` (empty `ACCEPTED` → rejection path),
  `ServerSession.Collectors.cs:1756-1767` (server side, same rule).
