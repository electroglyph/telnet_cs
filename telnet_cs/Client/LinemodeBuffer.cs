...
public sealed class LinemodeBuffer
{
    ...
    public LinemodeEdit Feed(char character)
    {
        ...
        switch (b)
        {
            case var eraseCharacter when eraseCharacter == SlcValue(LinemodeProtocol.SlcEraseCharacter):
                if (line.Length == 0)
                {
            ...
            case var eraseLine when eraseLine == SlcValue(LinemodeProtocol.SlcEraseLine):
                string echo = string.Concat(Enumerable.Repeat("\b \b", line.Length));
                line.Clear();
                return new LinemodeEdit(echo, null);

            case var eraseWord when eraseWord == SlcValue(SlcEraseWord):
                int popped = 0;
                ...
                return new LinemodeEdit(string.Concat(Enumerable.Repeat("\b \b", popped)), null);

            case '\r' or '\n':
            case var forwarded when forwardMask.Contains(forwarded):
