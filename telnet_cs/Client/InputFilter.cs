...
public sealed class InputFilter
{
    ...
    public static IReadOnlyList<KeyValuePair<byte[], byte[]>> PetsciiSequences { get; } = BuildSequences(
      ...
      ([0x1B, 0x4F, 0x43], [0x1D]),
      ([0x1B, 0x4F, 0x44], [0x9D]),
      ([0x1B, 0x5B, 0x33, 0x7E], [0x14]),
      ([0x1B, 0x5B, 0x48], [0x13]),
      ([0x1B, 0x5B, 0x32, 0x7E], [0x94]));
    ...
    private bool StartsWith(int offset, byte[] key)
    {
        ...
        return CollectionsMarshal.AsSpan(pending).Slice(offset, key.Length).SequenceEqual(key);
    }

    private bool IsProperPrefix(int offset)
    {
        int remaining = pending.Count - offset;
        var tail = CollectionsMarshal.AsSpan(pending).Slice(offset, remaining);
        foreach (var entry in sequences)
        {
            ...
            if (tail.SequenceEqual(entry.Key.AsSpan(0, remaining)))
            {
                return true;
            }
        }
