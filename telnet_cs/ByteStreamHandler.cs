namespace telnet_cs
{
  ...
  public partial class ByteStreamHandler : IByteStreamHandler
  {
    ...
    /// </summary>
    public NegotiationState Negotiation { get; set; } = new();
    ...
    /// </summary>
    public string TerminalType { get; set; } = Client.TerminalType;
    ...
    /// <summary>
    /// Gets or sets whether inbound LINEMODE MODE masks are answered with
    /// the server rules (<see cref="LinemodeState.ApplyModeAsServer"/>)
    /// instead of the client rules. Fed by the server session; false (the
    /// default) keeps the client behavior. SLC handling is role-symmetric
    /// and needs no flag.
    /// </summary>
    internal bool LinemodeServerRole { get; set; }
    ...
    /// before each read.
    /// </summary>
    public IReadOnlyDictionary<string, string> EnvironmentUserVars { get; set; } =
...
#pragma warning restore CA1031 // Do not catch general exception types
    ...
    /// rules when <see cref="LinemodeServerRole"/> is set.
    /// </summary>
    ...
    private Task ReplyModeAsync(List<byte> payload)
    {
      ...
      byte? reply = LinemodeServerRole ? Linemode.ApplyModeAsServer(payload[1]) : Linemode.ApplyMode(payload[1]);
      if (reply is null)
      {
        return Task.CompletedTask;
      }
