...
public class TelnetServerOptions
{
    ...
    /// executable offer list (US-ASCII last). Deliberately broader than
    /// <see cref="TelnetClientOptions.CharsetOffers"/> (3 entries):
    /// the server role must match whatever a peer requests — including
    /// retro codepages — while the client role only needs to offer the
