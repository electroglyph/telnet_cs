...
internal static class LinemodeProtocol
{
    ...
    /// <summary>
    /// Builds the SLC import-request triplet body (func 0 + DEFAULT).
    /// Shared by the client/server import requesters.
    /// </summary>
    /// <returns>The four payload bytes after the LINEMODE framing.</returns>
    internal static byte[] BuildSlcImportPayload() =>
      [SetLocalCharacters, 0, LevelDefault, 0];

    /// <summary>
    /// Builds the SLC export payload for <paramref name="state"/>, or null
    /// when nothing is configured. Shared by the client/server exporters.
    /// </summary>
    /// <param name="state">The SLC table to export.</param>
    /// <param name="forImport">Whether the export answers an import request.</param>
    /// <returns>The payload bytes, or null.</returns>
    internal static byte[]? BuildSlcExportPayload(LinemodeState state, bool forImport = false)
    {
        ArgumentNullException.ThrowIfNull(state);
        return state.ExportTriplets(forImport) is { } triplets
          ? [SetLocalCharacters, .. triplets]
          : null;
    }

    /// <summary>
    /// Determines whether a triplet is an import request (func 0 at
    /// DEFAULT or VALUE level, low two bits only — ACK/flush ride along).
    /// </summary>
    /// <param name="function">The SLC function code.</param>
    /// <param name="modifier">The SLC modifier byte.</param>
    /// <returns>True for an import request triplet.</returns>
    internal static bool IsSlcImportRequest(byte function, byte modifier) =>
      function == 0 && (byte)(modifier & LevelBits) is LevelDefault or LevelValue;

    /// <summary>
    /// Enumerates SLC triplets in a LINEMODE payload whose first byte is
    /// the subcommand. Caller guarantees <c>(Count-1)%3==0</c>.
    /// </summary>
    /// <param name="payload">The LINEMODE payload.</param>
    /// <returns>The (function, modifier, value) triplets.</returns>
    internal static IEnumerable<(byte Function, byte Modifier, byte Value)> EnumerateSlcTriplets(IReadOnlyList<byte> payload)
    {
        ArgumentNullException.ThrowIfNull(payload);
        for (int i = 1; i + 2 < payload.Count; i += 3)
        {
            yield return (payload[i], payload[i + 1], payload[i + 2]);
        }
    }

    /// <summary>
    /// Appends one SLC triplet to <paramref name="to"/>.
    /// </summary>
    /// <param name="to">The destination.</param>
    /// <param name="function">The SLC function code.</param>
    /// <param name="modifier">The SLC modifier byte.</param>
    /// <param name="value">The SLC value byte.</param>
    internal static void AppendSlcTriplet(List<byte> to, byte function, byte modifier, byte value)
    {
