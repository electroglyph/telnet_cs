  ...
  public partial class Client : BaseClient, IClient
  {
    ...
    public async Task WriteAsync(string command)
    {
      ...
      if (Settings.TextEncoding != null)
      {
        ...
        await WriteAsync(ByteStringConverter.ConvertStringToByteArray(command, Settings.TextEncoding)).ConfigureAwait(false);
        return;
      }
    ...
    /// <inheritdoc/>
    public async Task SendCommand(Commands command)
    {
      switch (command)
      {
        case Commands.Break:
      ...
      if (ByteStream.Connected && !InternalCancellation.Token.IsCancellationRequested)
      {
        ...
        try
        {
          ...
          await ByteStream.WriteAsync([(byte)Commands.InterpretAsCommand, (byte)command], 0, 2, InternalCancellation.Token).ConfigureAwait(false);
        }
        finally
        {
          SendRateLimit.Release();
    ...
    /// <inheritdoc/>
    public async Task SendSynchAsync()
    {
      ...
      if (ByteStream.Connected && !InternalCancellation.Token.IsCancellationRequested)
      {
        ...
        try
        {
          ...
          await stream.SendUrgentAsync((byte)Commands.Data, InternalCancellation.Token).ConfigureAwait(false);
        }
        finally
        {
          SendRateLimit.Release();
    ...
    {
      Guard.AgainstNullArgument(nameof(terminator), terminator);
    ...
    {
      Guard.AgainstNullArgument(nameof(regex), regex);
    ...
    {
      Guard.AgainstNullArgument(nameof(terminators), terminators);
    ...
    {
      Guard.AgainstNullArgument(nameof(regexes), regexes);
