...
public partial class ByteStreamHandler
{
    ...
    private string DecodeResult(StringBuilder sb, List<byte> rawBytes)
    {
        ...
        if (TextEncoding is not null)
        {
            ...
            ReadOnlySpan<byte> bytes = CollectionsMarshal.AsSpan(rawBytes);
            char[] rented = ArrayPool<char>.Shared.Rent(TextEncoding.GetMaxCharCount(bytes.Length));
            try
            {
                int count = textDecoder.GetChars(bytes, rented, flush: false);
                return new string(rented, 0, count);
            }
            finally
            {
